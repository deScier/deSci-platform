...
export default function ArticlesForApprovalPage() {
   ...
   const { data: articles, isLoading: articles_loading } = useSWR(authenticated_url(session, '/journals/documents'), fetcher_authenticated(session)) as {
      data: DocumentProps[] | undefined
      isLoading: boolean
   }

   const { data: journalsData, isLoading: journals_loading } = useSWR(authenticated_url(session, '/journals'), fetcher_authenticated(session)) as {
   ...
   React.useEffect(() => {
      if (articles) {
         ...
         if (debouncedSearchTerm) {
            filtered = filtered.filter((article) => article.title.toLowerCase().includes(debouncedSearchTerm.toLowerCase()))
         }

         if (status && status !== 'all') {
            if (status.toLowerCase() === 'approved') {
               filtered = filtered.filter((article) => article.status.toLowerCase() === 'approved' || article.status.toLowerCase() === 'submitted')
            } else {
...
const ArticleForApproval: React.FC<ArticleForApprovalProps> = ({ link, status, image, title, createdAt }: ArticleForApprovalProps) => {
