  ...
  React.useEffect(() => {
    ...
    if (originatesFrom) {
      filteredJournals = filteredJournals.filter(
        (journal) => journal.originatesFrom?.toLowerCase() == originatesFrom?.toLowerCase()
      );
    }

    if (debouncedSearchTerm) {
      filteredJournals = filteredJournals.filter((journal) =>
        journal.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
      );
    }
...
const JournalUnderReview: React.FC<JournalUnderReviewProps> = ({
  since,
  link,
  status,
  image,
