...
const TruncateWithHoverCard: React.FC<TruncateWithHoverCardProps> = ({
  text,
  className,
  truncateLength,
}: TruncateWithHoverCardProps) => {
        ...
        <HoverCardTrigger className="flex flex-col md:flex-row md:items-center gap-4 flex-1 min-w-0">
          <p
            className={cn(
              ...
              className,
              windowDimension > 1024 && windowDimension < 1080
                ? `max-w-[${truncateLength ? truncateLength : 30}ch]`
                : ''
            )}
          >
            {text}
          </p>
          <p className={cn('block lg:hidden hover:underline transition-all duration-200 truncate', className)}>
            {text}
          </p>
        </HoverCardTrigger>
