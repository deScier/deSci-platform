      ...
      <Popover open={open} onOpenChange={setOpen}>
         ...
         <PopoverContent className="p-0 w-full" style={{ width: btn_ref.current?.clientWidth, minWidth: '174px' }}>
            <Command>
               ...
               <CommandGroup>
                  {options.map((option) => (
                     <CommandItem key={option.value} value={option.value} className="text-sm" onSelect={() => handleSelect(option)}>
                        {option.label}
                        <CheckIcon className={cn('ml-auto h-4 w-4', value && value.value === option.value ? 'opacity-100' : 'opacity-0')} />
                     </CommandItem>
                  ))}
               </CommandGroup>
            </Command>
