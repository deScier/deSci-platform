export { SubmitedItem, Badge };
export type { SubmitedItemProps };
...
const SubmitedItem: React.FC<SubmitedItemProps> = ({ id, title, date, status, onClick }: SubmitedItemProps) => {
  return (
    <React.Fragment>
      <p
        className="text-sm cursor-pointer hover:text-primary-hover hover:underline transition-all duration-200"
        onClick={onClick}
      >
        {title}
      </p>
 ...
 * @param {string} props - The properties passed to the Badge component.
 */
const Badge: React.FC<{ status: string }> = ({ status }: { status: string }) => {
